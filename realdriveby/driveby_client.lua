...
function toggleDriveby()
	...
	if getElementData(localPlayer,"grabbed") then return end
...
function switchDrivebyWeapon(key,progress)
	...
	local switchTo
	localPlayer:Action_FastSelection(progress)


	local validWeapon = false

	setTimer(function ()
		for key, value in ipairs(weaponsTable) do
			if value == getPedWeapon(localPlayer) then
				validWeapon = true
				break
			end
		end	
