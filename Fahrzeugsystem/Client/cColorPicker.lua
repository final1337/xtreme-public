...
function openPicker(id, start, title)
  if id and not pickerTable[id] then
    ...
    return true
  elseif pickerTable[id] then
    pickerTable[id].color.h, pickerTable[id].color.s, pickerTable[id].color.v = rgb2hsv(getColorFromString(start))
    pickerTable[id]:updateColor()
  end
  return false
end
...
function colorPicker.create(id, start, title)
  ...
  cp.gui.okb = guiCreateButton(336, 235, 64, 24, "Okay", false, cp.gui.window)
